...
bool matchMessageToThisDevice() {
  if (incomingCanMsg.can_id == homebaseCanId) { //from home base
      if (incomingCanMsg.data[0] == myCanId || incomingCanMsg.data[0] == 0x00) { // me or all devices
        return true;
      }
  }
  return false;
...
bool processMessage() {  
  if (matchMessageToThisDevice() == true) {
    if (incomingCanMsg.data[1] == 0x0F) { //enable
      setRelayState(true);
      return 0; //don't send a message back
    } else if (incomingCanMsg.data[1] == 0x01) { //disable
      setRelayState(false);

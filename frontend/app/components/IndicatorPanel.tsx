...
const SensorsPanel: React.FC<{
    ...
    const triggeredDevicesWithinArmCycle = useSelector(function (state: AppStateSlice) {
        return state.appState.status.everTriggeredWithinArmCycle;
    });
    ...
    return (
        <Panel className={className}>
            <div className={"no_select"} style={{zIndex: 1, position: "absolute", top: 0, left: 5}}>{`${deviceList.length} sensors`}</div>
            {/* <div className={"no_select"} style={{zIndex: 1, position: "absolute", top: -2, right: 5}}>{`Selected profile: ${myAlarmProfile?.name}`}</div> */}
            {deviceList.map(
                (sensorElement, index) => {
